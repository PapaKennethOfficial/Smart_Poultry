...
import SupplyVsDemandPanel from '../components/SupplyVsDemandPanel'
...
function DemandForecastPanel() {
  ...
  return (
    <div className="chart-card" style={{ marginBottom: 16 }}>
      ...
      <ExplainChart chartId="demand_forecast" />
    </div>
  )
}

// ─── Morning Briefing (LLM) ──────────────────────────────────────────────────
    ...
    <div className="chart-card" style={{ marginBottom: 16, background: 'linear-gradient(180deg, #f8fbf8 0%, #ffffff 100%)' }}>
      <div className="chart-header">
        <div>
          ...
          <div className="chart-subtitle">Plain-language summary of the last 7 days</div>
        </div>
        <button
          className="btn-outline"
          style={{ padding: '5px 11px', fontSize: '0.75rem' }}
      ...
      {query.isLoading || (query.isFetching && !query.data) ? (
        ...
        <div style={{
          ...
          <div>
            {apiErrorMessage(query.error, 'Morning Briefing unavailable.')}
          </div>
        </div>
      ) : (
        <p style={{
...
function InsightCard({ icon: Icon, color, bg, title, value, desc, loading }) {
  return (
    <div style={{
      background: '#fff', borderRadius: 'var(--r-md)', padding: '13px 15px',
      border: '1px solid #e8ede8', display: 'flex', gap: 11, alignItems: 'flex-start'
    }}>
      <div style={{
        width: 32, height: 32, borderRadius: 'var(--r-sm)', background: bg,
        display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0
      }}>
        {loading ? (
          <Loader2 size={15} color={color} className="spin" />
        ) : (
      ...
      <div>
        ...
        <div style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: '1.3rem', fontWeight: 800, color: '#0a260d', margin: '2px 0 4px' }}>
          {loading ? '...' : value}
        </div>
...
function FulfilmentFunnelPanel() {
  ...
  return (
    <div className="chart-card">
      ...
      <ExplainChart chartId="fulfilment_funnel" />
    </div>
  )
}
...
function DriverEfficiencyPanel() {
  ...
  return (
    <div className="chart-card">
      ...
      <ExplainChart chartId="driver_efficiency" />
    </div>
  )
}
...
function OrderHeatmapPanel() {
  ...
  return (
    <div className="chart-card">
        ...
        <div style={{ overflowX: 'auto' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '38px repeat(24, minmax(18px, 1fr))', gap: 2 }}>
            ...
            {payload.grid.map((row, dayIdx) => (
              // Fragment needs the key, not its children — React warns
              // otherwise and rows can misrender if the grid reorders.
              <Fragment key={`row-${dayIdx}`}>
                <div style={{ fontSize: '0.7rem', color: '#0a260d', fontWeight: 500, alignSelf: 'center' }}>
                  {dayLabels[dayIdx]}
                </div>
      ...
      <ExplainChart chartId="order_heatmap" />
    </div>
  )
}
...
export default function Analytics() {
  ...
  const avgPredicted = forecast.length > 0
    ? Math.round(forecast.reduce((sum, item) => sum + (item.actual ?? item.predicted ?? 0), 0) / forecast.length)
    : 0
  ...
  return (
    <div>
      ...
      {/* Supply vs Demand sits first: it is the only view that connects the
          farm half to the commerce half, and it frames everything below it. */}
      <SupplyVsDemandPanel />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14, marginBottom: 24 }}>
        <InsightCard
          ...
          color="#2e7d34" 
          bg="#f0f7f0" 
          title="Egg Output (7-day avg)" 
          value={insights?.recentYield ? `${insights.recentYield.value.toLocaleString()} eggs` : '—'} 
          desc={insights?.recentYield ? `Daily average this week, ${insights.recentYield.change} vs last week` : '—'} 
        ...
        <InsightCard 
          ...
          desc={insights?.fcrStatus ? `kg feed per kg eggs · ${insights.fcrStatus.status} (${insights.fcrStatus.benchmark})` : '—'} 
          loading={loadingInsights}
        />
        <InsightCard 
          icon={Zap} 
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
        ...
        <div className="chart-card">
          <div className="chart-header">
            <div>
              <div className="chart-title">Egg Collection — Last 10 Days</div>
              <div className="chart-subtitle">Recorded daily totals from the logbook (measured, not predicted)</div>
            </div>
            <span className="badge badge-gray">Actuals</span>
          </div>
            ...
            <>
              <ResponsiveContainer width="100%" height={220}>
                <ComposedChart data={forecast} margin={{ top: 5, right: 10, left: -10, bottom: 0 }}>
                  ...
                  <YAxis tick={{ fontSize: 10, fill: '#7a917b' }} axisLine={false} tickLine={false} />
                  <Tooltip content={<CustomTooltip />} />
                  <Area type="monotone" dataKey="actual" stroke="#2e7d34" strokeWidth={2.5}
        ...
        <div className="chart-card">
          <div className="chart-header">
            <div>
              ...
              <div className="chart-subtitle">kg feed per kg eggs, weekly, vs industry benchmark</div>
            </div>
            {insights?.fcrStatus?.verdict === 'better' ? (
              <span className="badge badge-green">Better than benchmark ↓</span>
            ) : insights?.fcrStatus?.verdict === 'worse' ? (
              <span className="badge badge-red">Worse than benchmark ↑</span>
            ) : (
          ...
          {loadingFcr ? (
            ...
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={fcrData} margin={{ top: 5, right: 10, left: -10, bottom: 0 }}>
                ...
                <ReferenceLine
                  y={insights?.fcrStatus?.benchmark ?? 2.3}
                  stroke="#d03b3b"
                  strokeDasharray="3 3"
                  label={{
                    value: `Benchmark (${insights?.fcrStatus?.benchmark ?? 2.3})`,
                    fill: '#d03b3b', fontSize: 10, position: 'top',
                  }}
                />
              </BarChart>
            </ResponsiveContainer>
          )}
          <ExplainChart chartId="fcr" window={6} />
        </div>
      </div>
